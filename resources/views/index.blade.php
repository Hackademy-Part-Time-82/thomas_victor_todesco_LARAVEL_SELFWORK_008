        ...
        <div class="row justify-content-center">
            <div class="col-lg-9">
                ...
                <div class="container-lg card card-lumen">
                    <div class="row justify-content-around gap-3 mt-5">
                        @foreach ($books as $book)
                            <div class="card col-lg-3 p-1 text-center">
                                <img src="{{$book->image ? Storage::url($book->image) : '\storage\covers\generic_cover.jpg'}}" class="card-img-top img-fluid" alt="..." width="560" height="350">
                                <div class="card-body border-black border-top">
                                    <h5 class="card-title">{{ $book->title }}</h5>
                                    <p class="card-text">Pagine: {{ $book->pages ?? 'N/D' }}</p>
                                    <a href="{{ route('show', ['book'=>$book]) }}" class="btn btn-primary">Vai al dettaglio</a>
                                </div>
                            </div>
                        @endforeach
                    </div>
